...
## Latest checkpoint — local QR accepted, integration branch Git handoff (4 September 2026)

- User confirmed local testing on their device works, specifically QR processing, and explicitly authorized committing/pushing the current branch. This is user-reported acceptance, not a new automated or cloud test. No further feature changes, tests, backend restart, main merge or manual deployment in this handoff.
- Commit scope: native camera/image QR import, native preview/explicit save, read-only backend import endpoint, richer PDF resume snapshots, retained lifecycle/day-chip fixes, formatting-test source and migration/QA documentation, integrated with team head `1eacf55` (UI/theme/community ratings).
- Before commit, fetched origin and confirmed local HEAD matches origin/integration/merge-all. Keep both pre-merge backup stashes. Commit/push only `integration/merge-all`; do not force-push or change main.
- Exclude ignored local.properties (personal LAN IP/SDK/signing settings), .venv, APK/build output, .run logs, tmp fixtures/backups and local databases. Local connection remains configured for S23; committing code does not publish/migrate its local QR tokens or community records to the cloud database.
- Deployment remains a separate follow-up. No Render management action is performed here; the host's own auto-deploy setting may react to a push with backend changes. Do not claim the hosted QR endpoint works until it is explicitly verified after deployment.

## Previous checkpoint — S23 connected to local backend for user testing (4 September 2026)

- User explicitly chose local QR testing on S23 and authorized connection/setup only; they will test and confirm before commit/push. Do not run functional tests, commit, push or deploy without their follow-up.
- S23 `SM-S911B`, ADB serial `RFCX80S1FJN`, USB authorized. Phone Wi-Fi `192.168.0.193`; PC Wi-Fi is now `192.168.0.129`. Updated only ignored local.properties `journify.devServerIp` from `192.168.1.10` to `192.168.0.129`, preserving SDK settings. No ADB reverse or firewall changes.
- Built debug successfully (3s); BuildConfig confirms applicationId `com.journify.app` and PHONE_BASE_URL `http://192.168.0.129:8000/`. Installed with `adb install -r` successfully and launched `com.example.finalproject.presentation.MainActivity`. Old `com.example.finalproject` app/data were not removed. The new package uses separate local storage.
- Backend was not listening, so started local `.venv` Python with `manage.py runserver 0.0.0.0:8000 --noreload`, hidden process. Launcher PID 9032, Python child/listener PID 5028 (verify live ownership before stopping). Logs: `backend/.run/s23-20260904-155253.out.log` and `.err.log`. Django system checks passed, server started, AI model/cache loaded. Leave running for user testing; no migrations or dependency installs performed.
- Connection-only probe from S23 to local `/api/mobile/itineraries/import/check/` returned the expected JSON HTTP 400 INVALID_QR for the deliberately short dummy token. This verifies phone-to-backend reachability and the local QR endpoint, not a successful real itinerary import; a corresponding 400 in the log is intentional. No itinerary/account mutations or automated QR tests performed.
- Earlier read-only cloud check: Render catalog API responded 200, but the cloud QR import path returned an HTML 404 (route absent). Local-only QR backend work is still uncommitted/unpushed. Keep using local backend for this test; cloud deployment requires later explicit direction.

## Previous checkpoint — second team merge: UI/theme/community ratings, build passed (4 September 2026)

- User approved the same backup/merge/build-only workflow. No test, emulator install/launch, commit, push, backend restart, migration or deployment requested or performed.
- Fetched and fast-forwarded local `integration/merge-all` from `f54bbfb` to `1eacf55`. Five incoming commits include refreshed UI/fonts/home carousel, light/dark mode, community star ratings and rating-format/RecyclerView crash fixes. No backend or dependency-file changes in this incoming range; main remains untouched.
- Backed up all local tracked/untracked changes in retained stash `42c04603b525ff04003e5dccb09bea9c1c1827b9` (`backup-local-before-ui-rating-merge-2026-09-04`). Applied onto the new head without dropping this or the earlier backup.
- Resolved two conflicts: MainActivity keeps BOTH the team's theme switch and local restored-toolbar-title handling; ItineraryActivity keeps BOTH communityId/rating actions and QR preview/imported-save handling. QR files/API/docs, text sharing, community publish, prior snapshot enrichment and day-chip fixes remain present. Updated upstream layout styles were retained alongside the QR import entry/save button.
- Initial build found 13 duplicate English string resources because the team now supplies those translations in values-en/strings.xml. Removed the redundant declarations from local values-en/strings_integration.xml, preserving the team's definitions. The local supplementary resource file is now just a comment; no translations were lost.
- Final `:app:assembleDebug --console=plain` completed BUILD SUCCESSFUL in 23s (35 tasks: 16 executed, 19 up-to-date), with only deprecated-API notes. APK: `app/build/outputs/apk/debug/app-debug.apk`, applicationId remains `com.journify.app`. Git conflict index resolved; whitespace/conflict-marker checks complete. Combined changes remain uncommitted in the working tree, backup stashes retained, main untouched. No runtime test, unit test, lint or installation: user will test this new combined APK; earlier emulator results are not evidence for this version.

## Previous checkpoint — team updates merged with local QR work, build passed (4 September 2026)

- User requested combining the latest team integration branch with this machine's current version, BUILD ONLY so they can test themselves. No runtime testing, emulator installation/launch, unit tests, lint, backend commands, migration, deployment or push in this turn.
- Fetched `origin/integration/merge-all` and fast-forwarded local `integration/merge-all` from `fad080b` to `f54bbfb`. This includes the team's community publish/session/list/detail/budget/image fixes and NaN rating fix, plus offline/auth/UI changes. `main` was not changed.
- Before updating, backed up all tracked/untracked local changes with retained stash `ba8473344cca420ebeb9ccd12fc94ddfeea30065` (`backup-local-qr-before-team-merge-2026-09-04`). Applied it onto the updated branch; did not pop/drop the backup. Local QR source/docs remain uncommitted in the working tree.
- Resolved ProfileFragment overlap by retaining the team's null-safe text helper (covers the local null-phone/avatar fix). Combined the Trips QR header with the team's signed-out panel inside the list content container. Preserved camera/image import, QR preview/save, day-chip restoration, snapshot fields/import API, text share AND community publish.
- Removed duplicate definitions from local integration resource files where the team now defines them in strings.xml; retained 13 missing English translations. The existing formatting-test source now reads strings.xml; tests were NOT run. Weather callback fix is already included upstream.
- IMPORTANT: upstream changes applicationId to `com.journify.app` (Java namespace remains `com.example.finalproject`). A newly installed build is a separate app from the old emulator package and does not inherit its Room data/cookies. The old app/data have not been removed. Launcher is now MainActivity with guest browsing. Preserve these intentional team changes.
- Upstream adds a hosted release backend default, but this machine's ignored `journify.devServerIp=192.168.1.10` is unchanged: debug uses local backend (emulator 10.0.2.2:8000, phone configured LAN); release has upstream HTTPS configuration. No deployment is performed or verified by this merge.
- Verification complete: `:app:assembleDebug --console=plain` finished BUILD SUCCESSFUL in 11m 27s, 35 tasks executed, using Java at `D:\AndroiStudio\jbr` and the existing Gradle cache. Output: `app/build/outputs/apk/debug/app-debug.apk`, applicationId `com.journify.app`. Only deprecation/build warnings, no compilation errors. Git conflict index is resolved and diff whitespace check passes. No tests or installation were run; user will test this combined APK. Previous QR/emulator test results below are for the earlier APK, NOT this newly combined version.

## Previous checkpoint — QR import emulator verification (4 September 2026)

- User explicitly requested emulator testing after the feature implementation. Installed the already-built APK with `adb install -r` on `emulator-5554`, preserving app data. Existing backend on port 8000 already serves the import endpoint; no restart or migration was needed. No app-source fixes were required in this test turn, no commit/push, no community testing.
- Confirmed actual old PDF QR PNG opens correct 3-day preview with 1,377,000 VND estimated / 1,623,000 remaining. Day switching works. Leaving preview does not save; explicit Save returns to Trips and the saved result reopens correctly.
- Retained synthetic imported trip `QA0902-Integration • Hoi An Quang Noodles`, saved 04/09 at 11:40, for user inspection. Both pre-existing visible saved trips remain untouched.
- Confirmed camera deny/allow/open/close, invalid QR, no QR, multiple QR, missing token, network failure, retained error/retry state over rotation, successful retry after reconnection without image reselection, and VI/EN labels. No crash-buffer entries in this test's 11:xx period.
- Restored emulator Wi-Fi/data and rotation settings to their original values, restored virtual-scene poster to default. Camera permission was granted through the test flow. Backend remains running. QA fixture images are in emulator Downloads and ignored local tmp.
- Camera optical decode remains unverified: the virtual camera scene did not place a QR within view. Expired token, corrupt snapshot, newly exported PDF round-trip, JPEG/huge images, large text/multiple devices and full-app regression are not claimed tested. See `QR_IMPORT_TEST_REPORT.md` and updated `QR_IMPORT.md` for exact evidence and boundaries. Previous unit/lint results below do not represent a new run for QR.

## Previous checkpoint — native QR itinerary import (4 September 2026)

- User approved adding camera scanning and selecting a QR image, with polished UI. Community work is explicitly left to the teammate; no community source/API was edited. Existing uncommitted changes are preserved on `integration/merge-all`; no commit/push.
- Entry in Trips opens a bilingual green/cream import screen with camera/image actions, permission guidance, loading/error/retry states and privacy instructions. ZXing Android Embedded 4.3.0 supplies camera/QR decoding; images are sampled to at most 2048 px and decoded locally, never uploaded. ViewModel owns background work over rotation.
- New read-only GET `/api/mobile/itineraries/import/<token>/` returns the stored snapshot as the Android itinerary contract. Expired/missing/invalid snapshots have distinct HTTP errors. Reads existing QR records; no migration, matrix edit, catalog edit, web-session mutation or community publishing.
- QR input is parsed for a strict resume token; scanned hosts are never fetched. App calls only its configured backend, allowing emulator-host QR codes to be imported on a phone connected to the same backend/database. This does not make local server URLs publicly reachable.
- Imported trips open native ItineraryActivity with a not-yet-saved badge and an explicit Save to my trips action. Existing Room save/navigation is reused; no automatic save. New PDF snapshots retain per-stop prices/details, while older tokens use catalog enrichment for missing fields.
- Verification: Python syntax compilation succeeded for the three backend files; final `:app:assembleDebug` completed BUILD SUCCESSFUL in 1m 35s after the UI polish (initial dependency build also passed). Output: `app/build/outputs/apk/debug/app-debug.apk`. `git diff --check` passed. No unit tests, lint, device installation, camera tests, image-import runtime tests or backend server/database changes were run in this feature turn, respecting the user's earlier request to self-test. See `QR_IMPORT.md` for usage, boundaries and a manual checklist.

## Previous checkpoint — build-only verification (4 September 2026)

- User requested fixing build only, explicitly deferring all tests and emulator testing to themselves. This turn did not run unit tests, lint, backend, install, launch, or emulator actions.
- Current checkout is still `integration/merge-all`; earlier uncommitted source/resource fixes were preserved. No new application-code changes were needed, and no commit/push was performed.
- `:app:assembleDebug --console=plain --stacktrace` completed BUILD SUCCESSFUL in 23 seconds using the bundled Java at `D:\AndroiStudio\jbr` and the user's Gradle cache. Output: `app/build/outputs/apk/debug/app-debug.apk` (9,603,384 bytes). The tool-shell environment has no JAVA_HOME/Java on PATH by default; the successful invocation sets JAVA_HOME only for its process.
- Android Studio's actual Run logs show deployment failures, not compilation failures: at 10:44 `cmd: Can't find service: package`, followed at 10:45 by `LoginActivity does not exist`. The source manifest declares LoginActivity as launcher and its compiled class exists. Do not remove/rename the activity to mask the emulator/deployment error. Emulator recovery and installation remain unverified and were not attempted per the user's build-only scope.
- Earlier runtime findings and pending retests remain outstanding; this successful build is not a claim that all integrated features work.

## Previous checkpoint — integration test in progress (2 September 2026)

- User authorized pulling and full emulator testing; the earlier testing deferral is superseded. `git pull --ff-only origin integration/merge-all` returned already up to date at `fad080b`. Main is untouched.
- Emulator `emulator-5554` is connected. Django local backend was started for testing on port 8000. Django system check and all 8 existing tests pass (temporary test database).
- First Android build failed: `RemoteWeatherRepository` imported `domain.repository.RepositoryCallback` instead of `domain.callback.RepositoryCallback`. User explicitly approved fixing integration/build issues and continuing tests. Import corrected locally.
- Next build exposed missing string resources inherited from the merged branch. Added bilingual integration resources for auth, location, offline, weather, mood and saved-title labels; restored missing English auth/profile/community translations.
- After these fixes, `testDebugUnitTest assembleDebug lintDebug` succeeded: 25 Android unit tests passed, lint has 0 errors / 88 warnings. APK installed successfully with `adb install -r` on `emulator-5554`, preserving data. App-private databases/preferences backed up to ignored `tmp/emulator-before-upgrade.tar` first. Login screen opens; empty-field validation works. Other runtime flows are still in progress. No main merge or push authorized in this testing phase.

## Previous checkpoint — per-tester LAN configuration, testing pending (2 September 2026)
